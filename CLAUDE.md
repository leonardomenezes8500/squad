...
- **Compose, don't duplicate.** ponytail (simplicity), humanizer (prose), agent-skills (reviewer, test and security agents), graphify (codebase graph) and the Codex plugin (a second model) do their part; squad orchestrates them. Every skill still works when a companion is missing, with a plain fallback.
...
- Codex plugin: `/codex:review` and `/codex:adversarial-review` are user-only (`disable-model-invocation`), so squad runs the same reviews through the plugin's runtime (`codex-companion.mjs`) under the team skill's opt-in, and asks `codex:codex-rescue` for read-only help at breaking points. Its review gate (a Stop hook) stays off.
